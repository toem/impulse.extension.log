<!---
title: "JSON Log Reader"
author: "Thomas Haber"
keywords: [log, json, impulse, text log, parsing, serializer, extension, configuration, analysis, multi-object, hierarchy, tagging, domain, timestamp, Log4j, syslog]
description: "The JSON Log Reader extension for impulse enables flexible import and analysis of JSON-based logs using user-defined object and value mappings. Supports multi-format logs, hierarchical organization, timestamp extraction, multi-object handling, severity tagging, and advanced configuration for uniform visualization and processing across diverse JSON log sources."
...
The JSON Log Reader is designed to handle a wide variety of JSON-based log formats using user-defined object and value mappings. It is highly configurable and supports complex log parsing scenarios, but requires careful setup for best results.

The JSON Log Reader lets you import, parse, and analyze JSON log files in impulse by mapping JSON objects and values to log fields. It enables uniform visualization and processing of logs from diverse sources, even when JSON schemas differ due to historical or toolchain reasons.

With the JSON Log Reader, you can:
- Parse mixed-format JSON logs using multiple object/value patterns
- Extract log level, timestamp, logger/location, method, and message fields from JSON values
...
![](images/ss_json-log_dialog1.png)

## Dialog Sections and Fields

The JSON Log Reader dialog allows you to define how JSON log files are parsed and mapped into impulse signals. It is divided into several sections for flexible setup and testing.

### General Section

This section contains the main identification and categorization properties of the JSON log reader.
...
This section displays a table listing all defined JSON log entries.

**Columns:**
- **Name**: The name of the pattern (e.g., "Event").
- **Type**: The type of pattern (e.g., Log Json).
...
Json Log entries are evaluated top-to-bottom; the first matching pattern is applied.
...
This dialog allows you to define a single JSON log entry for the JSON Log Reader.

![](images/ss_json-log-option_dialog1.png)
...
- **Object**: The JSON object name/path to match (e.g., `log4j:event`).
- **Values**: Comma-separated list of JSON values to extract (e.g., `logger,timestamp,sequenceNumber,level,thread`).
...
Map JSON values to log fields:
- **Source**: The JSON value  (e.g., `logger`, `level`, `timestamp`, `thread`).
...
Suppose you have a JSON log file with entries like:
...
### Step 1: Create a JSON Log Reader

- Go to **impulse Preferences → Serializers**, then right click on the root element and add a **JSON Log Reader**.
- Set a name, select the character set, and set the time base (e.g., `ms`).

### Step 2: Add Json Log entry
...
- Import your JSON log file and analyze the structured signals in impulse.
...
This section documents the concrete JSON Log Reader (type 'reader.log.json') data model.

### Json Log Reader 'reader.log.json' 

| Field | Type | Default | Notes |
|---|---|---|---|
| name | string | null | Unique name of the json log reader. |
...
|---|---|---|---|
| Log JSON | reader.log.json.option | 0..n | Each `reader.log.json.option` entry defines a single log parsing json within the reader. |

### Log Pattern `reader.log.json.option`

#### Core Pattern Fields

| Field | Type | Default | Notes |
|---|---|---|---|
| name | string | null | Name of this log json (e.g., "Event", "ISR"). |
...
| enabled | boolean | true | Whether this json is active. |
| action | integer | 0 | Action when json matches: 0=Ignore, 1=Start new sample, 2=Add to previous, 3=Finish sample. |
| path | string | null | JSON object path pattern for selection. |
...
## Predefined Pattern Log Reader JSON Examples

Below are several predefined pattern log reader configurations in JSON format. They are useful for AI-assisted generation of new readers.

```json
    <reader.log.json name="Log4J JSON">
        <properties len="1">
            <relativeDomainValue>true</relativeDomainValue>
        </properties>
        <reader.log.json.option name="Event" warningPattern="WARN" action="1" tagSource="2" values="thread,level,loggerName,message,endOfBatch,loggerFqcn,threadId,threadPriority" errorPattern="ERROR" m4="Message" m2="Level" name1Source="3" m1="Thread" fatalPattern="FATAL" nameMode="1"/>
        <reader.log.json.option name="Timestamp" domainSource="1" domain2Source="2" warningPattern="warning|WARNING|Warning" path="instant" action="2" domain2Mode="2" domainUnit="s" domain2Unit="ns" values="epochSecond,nanoOfSecond" domainMode="2"/>
        <reader.log.json.option name="Ignore other" path="*"/>
    </reader.log.json>
    <reader.log.json name="GELF">
        <properties len="1">
            <relativeDomainValue>true</relativeDomainValue>
        </properties>
        <reader.log.json.option name="Event" domainSource="5" warningPattern="4" action="1" s2="4" s5="0" s6="4" s7="4" s9="4" domainUnit="s" tagSource="6" values="version,host,short_message,full_message,timestamp,level,facility,line,file,_thread,_logger" errorPattern="3" m8="Line" m9="File" m6="Level" m7="Facility" s10="4" m4="Full Message" m5="Timestamp" m2="Host" m3="Message" name1Source="2" fatalPattern="0|1|2" domainMode="1" m10="Thread" nameMode="1"/>
        <reader.log.json.option name="Ignore other" path="*"/>
    </reader.log.json>
